...
namespace KS.Desktop
{
    public static class Extension
    {
        ...
        {
            Configuracao _configuracao = new();

            if (File.Exists(combinedConfigPath))
                using (var sr = new StreamReader(combinedConfigPath))
                    _configuracao = JsonConvert.DeserializeObject<Configuracao>(Base64Decode(sr.ReadToEnd()));

            configuracao.Parametros = _configuracao.Parametros;
        }
